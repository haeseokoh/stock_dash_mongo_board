        ...
        dbc.Row([
            dbc.Col([
                html.Div([
                    ...
                    dbc.Row([
                        dcc.Graph(id='OHLCV-fig3'),
                    ]),
                    dbc.Row([
                        dcc.Graph(id='trading-fig3'),
                    ]),
                ],),
            ], width=4, style={'margin': '35px'}),
            dbc.Col([
                html.Div([
                    html.Iframe(id='iframe-trading3',
...
@app.callback(
    dash.dependencies.Output('iframe-trading3', 'src'),
...
@app.callback(
    dash.dependencies.Output('OHLCV-fig3', 'figure'),
    dash.dependencies.Output('trading-fig3', 'figure'),
    Input('select-cell-momentem-data', 'children'),
)
def plm_report_sink_main(select_cell_data):
# def plm_report_sink_main(select_cell_data, select_group_sub_cell_data):
    ...
    if select_cell_data is not None:
        jongmok = select_cell_data.split(' *')[0] #'CJ제일제당'
    else:
        jongmok = None
